...
train_split = int(len(df_total)*0.7)
...
model = Sequential()
model.add(LSTM(8, input_shape = (1,24),activation='relu'))
#model.add(Dropout(0.2))
#model.add(Dense(4)) #activation='relu'#model.add(Dense(8))
model.add(Dense(1))
model.compile(loss='mean_squared_error', optimizer='adam')
model.fit(X_train, y_train, epochs=600, batch_size=10, verbose=2)
...
trainScore_2 = np.sqrt(mean_squared_error(df_total['GDP_Shift'], totalPredict))
print('Full Set Score: %.2f RMSE' % (trainScore_2))
#testScore = math.sqrt(mean_squared_error(testY[0], testPredict[:,0]))
#print('Test Score: %.2f RMSE' % (testScore))

final_df = pd.DataFrame(df_total['GDP_Shift'])
final_df['pred'] = totalPredict

final_df.plot()

#Score = model.evaluate(df_total['GDP_Shift'], totalPredict, verbose=0)
#print (' Score: %.2f MSE (%.2f RMSE)' % (Score, sqrt(Score)))
